...
type Trial struct {
	...
	Seed                      int
	RequestID                 *string
	BestValidationID          *int
	RunnerState               string // TODO(ilia): enum for training/validating/checkpointing.
	TaskID                    *string
	RunID                     int
	Restarts                  int
	Tags                      map[string]string // Note: values are always "".
	CheckpointSize            int
	CheckpointCount           int
	SearcherMetricValue       *float64
	SearcherMetricValueSigned *float64
	TotalBatches              int
	SummaryMetrics            map[string]any // TODO(ilia): better typing.
	SummaryMetricsTimestamp   *time.Time
	LatestValidationID        int
...
func MarkLostTrials(ctx context.Context) error {
	return db.Bun().RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		if err != nil {
			...
			// TODO(ilia): We can write to trial logs that they've been marked as errored
			// due to not being MIA / not reporting heartbeat within the timeout.
		}
