...
func createPodWithMockQueue(k8sRequestQueue *requestQueue) (
	...
	*actor.Ref,
	...
	// mockPodIface := &mockPodInterface{pods: pods}
	mockPodIface := &mocks.PodInterface{}

	configMapInterface := &mockConfigMapInterface{configMaps: make(map[string]*k8sV1.ConfigMap)}
	k8sRequestQueue = startRequestQueue(
		map[string]typedV1.PodInterface{"default": mockPodIface},
	...
	mockPodIface.On("Create", mock.Anything, mock.Anything, mock.Anything).Return(
		&k8sV1.Pod{}, nil).Run(func(args mock.Arguments) { pods[newPod.podName] = newPod.pod.DeepCopy() })

	mockPodIface.On("Delete", mock.Anything, mock.Anything, mock.Anything).Return(nil).Run(
		func(args mock.Arguments) { delete(pods, newPod.podName) })
	...
	return system, newPod, ref, podMap, actorMap, k8sRequestQueue.podInterfaces["default"], pods
}
...
/*
	...
	func TestReceiveContainerLog(t *testing.T) {
		...
		podInterface.On("GetLogs", mock.Anything, mock.Anything).Return(
...
func TestKillTaskPod(t *testing.T) {
	...
	/*
		THIS WORKS
		system, newPod, ref, _, _, _, pods := createPodWithMockQueue(nil)
	...
	*/
	...
	// THIS DOESN'T
	time.Sleep(time.Second)
	system, _, ref, podMap, _, _, _ := createPodWithMockQueue(nil)
...
func TestResourceCreationCancelled(t *testing.T) {
	...
	time.Sleep(time.Second)
	system, _, ref, podMap, _, _ := createPodWithMockQueue(k8sRequestQueue)
