...
def _default_storage_manager() -> storage.SharedFSStorageManager:
    base_path = Path(appdirs.user_data_dir("determined")) / "checkpoints"
...
def init(
    *,
    ...
    if not has_storage:
        tensorboard_mode = core.TensorboardMode.OFF
    ...
    if info.task_type == "TRIAL":
        ...
        if unmanaged:
            log_shipper = core.UnmanagedTrialLogShipper(
                session=session,
            ...
            if distributed and distributed.rank == 0:
                state_manager = core.UnmanagedTrialStateManager(
                    session=session,
                    trial_id=info.trial.trial_id,
                )
    else:
